...
* Sorry, did I say that `namedVector["name"]` works for subsetting?
  ...
  ```
  When you try to give an atomic vector names, R will only try to name it element-by-element (even if said vector has dimensions). Data frames, on the other hand, make no distinction between names and colnames. R ultimately sees named matrices as named atomic vectors that happen to have a second dimension. This means that you can subset them with both `["name"]` and `[, "name"]` and get different results.
  ```{r, echo = TRUE}
