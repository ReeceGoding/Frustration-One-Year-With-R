...
* I haven't done enough OOP in R. My only real experience of it is with S3. S4 looks enough like CLOS that I expect that I will go back to it at some point after picking up Common Lisp, but that will just be to play around.
...
* R is **the** language of research-level statistics. If it's stats, R either has it built-in or a has library for it. It's impossible to visit a statistics Q&A website and not see R code. For this reason alone, R will never truly die.
...
* `R Markdown` has served me well in writing this document. It's buggier than I'd like, rarely has helpful error messages, and does things that I can't explain or fix even after setting a bounty on Stack Overflow, but it's still a great way to make a document from R. It's the closest thing that I know of to an R user's LaTeX. As soon as [this bug](https://github.com/rstudio/rmarkdown/pull/2093) is fixed, I'll be numbering my sections.
...
* Basically everything is a vector, so R comes with some really cool vector-manipulation tools like `ifelse()` (as seen above) and makes it very easy to use a function on an entire collection. Can you believe that `mtcars / 20` actually works?
